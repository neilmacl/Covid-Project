...
CASE
	WHEN a."status" = 'PT' OR a."status" = 'FT' THEN a."status" 
	...
	WHEN AvgTotalPayBenefitsJobFT = 0 AND AvgTotalPayBenefitsJobPT = 0 AND ABS((Select AVG(TotalPayBenefits) FROM SFSalaryNew WHERE "status" = 'FT') - TotalPayBenefits) < ABS((Select AVG(TotalPayBenefits) FROM SFSalaryNew WHERE "status" = 'PT')- totalpaybenefits) THEN 'FT'
	ELSE 'PT'
